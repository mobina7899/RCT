...
function calFur15 (exit) {
    ...
    if($('#first_text').val() != ''){
        ...
        const mp1 = 1+ ((m()-1)*Rho1());
        const mp2 = 1+ ((m()-1)*Rho2());
        const pp1 = p1() * (1 - p1());
        const pp2 = p2() * (1 - p2());

        var cal = (Math.pow( ( zz ),2) * ((pp1 * mp1) + (pp2 * mp2)))/(dd);
        cal = Math.ceil(cal);
        ...
        if(exit){
            ...
            $('#hypothesis').val('');
            $('#scale').val('');
            $('#furmul15').fadeOut().css("display", "none");
        }
        listNumbers++;
    } else {
...
$(document).on('click', '#selectBtnExit15', ()=>{
    calFur15(true)
    window.scrollTo({top: 0, behavior: 'smooth'});
});
$(document).on('click', '#selectBtn15', ()=>{
    calFur15(false)
    window.scrollTo({top: 0, behavior: 'smooth'});
});
